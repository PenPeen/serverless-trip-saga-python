...
## 補足: リリースブランチ運用によるコスト最適化

### 背景

デフォルトの設計では `main` への Push がパイプラインのトリガーとなるため、PR をマージするたびにビルド（CodeBuild）が実行され、その分コストが発生します。個人開発など頻繁なリリースが不要なケースでは、不要なビルド実行を避けたいところです。

### 方針

PR のマージ先を `develop`（リリースブランチ）とし、リリースしたいタイミングで `develop` → `main` にマージする運用とします。パイプラインの設定変更は不要です。

### ブランチ戦略

```
feature/A ──→ develop にマージ   ← パイプライン動かない（$0）
feature/B ──→ develop にマージ   ← パイプライン動かない（$0）
feature/C ──→ develop にマージ   ← パイプライン動かない（$0）

「リリースしよう」
  develop → main にマージ        ← ここでパイプライン起動（ビルド1回分）
  → テスト → 承認 → デプロイ
```

### コスト比較

| 方式 | 月10回マージ・月2回リリースの場合 |
|---|---|
| PR を直接 main へ（デフォルト） | ビルド10回 × 5分 = 50分 → 約 $0.25 |
| **リリースブランチ運用** | **ビルド2回 × 5分 = 10分 → 約 $0.05** |

### 注意事項

*   `develop` ブランチでの CI（テスト実行）は GitHub Actions 等の無料枠を活用し、コード品質は別途担保することを推奨します。
*   `main` には直接コミットせず、必ず `develop` からのマージのみとする運用ルールを徹底してください。
